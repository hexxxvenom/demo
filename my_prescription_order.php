...
<div style="padding-top: 50px;font-family: 'Times New Roman',Times,serif;" >
		<div class="order">
			...
			<ul>
				<li><a href="my_order.php">ORDER</a><i>|</i></li>
		...
		<?php
			$uid=$_SESSION['usr_id'];
			$qry=mysqli_query($con,"SELECT * FROM `prescription` where `usr_id`='$uid'");
		...
		?>
		<?php
			$select=mysqli_query($con,"SELECT * from `prescription` where `usr_id`='$uid'");
		...
		?>
			<div class="orders" >
				<h5 style="float: right">Order Id:<?php echo $row['presc_id']; ?></h5>
				<h4 style="color: red"><?php echo $row1['m_name']; ?></h4>
				<h5 style="padding-top: 10px">Date:<?php echo $row['p_date']; ?></h5>
			</div>
		...
		?>
				<a href="my_prescription_detail.php?pid=<?php echo $row['presc_id']; ?>" style="float: right"><input type="submit" name="" value="View Details"></a>
			</div>
			<br>
		<?php
			}
